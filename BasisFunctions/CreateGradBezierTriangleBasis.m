function [dphidx,dphidy,wdetJ]=CreateGradBezierTriangleBasis(mesh_file,GaussPts)
    ...
    case 'nodes'
        ...
        for i1=1:prod(Nbselems)
            ...
            for isub=1:size(iconnr,1)
                ...
                ipix=conn(isubg,1:3);
                for in=1:10
                N_x=N_r(insub,in).*invJ(insub,1)+N_s(insub,in).*invJ(insub,3);
                N_y=N_r(insub,in).*invJ(insub,2)+N_s(insub,in).*invJ(insub,4);
                    id=inods(in);
                    indn(nel+(1:3))=id;
                    indp(nel+(1:3))=ipix;
                    valx(nel+(1:3))=N_x;
                ...
                nn(ipix)=nn(ipix)+(abs(sum(N(insub,:),2))>0);
            end
        end
        nn=1./max(1,nn);
        dphidx=sparse(indp,indn,valx,nx,ny);
        dphidx=diag(sparse(nn))*dphidx;
        dphidy=sparse(indp,indn,valy,nx,ny);
        dphidy=diag(sparse(nn))*dphidy;
