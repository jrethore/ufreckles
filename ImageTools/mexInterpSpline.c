...
void mexInterpSpline(double *Upp,double *Vpp,double *im1, mwSize m, mwSize n,double *im0, mwSize mc, mwSize nc)
{
    int ii,jj,IX1,IY1,mcnc,ind,count=0;
    double Xi,Yi,Xx,Yy,XxYy,NivGris;
...
/*             Xi=*(Upp+count)+ii+1;
            ...
            XxYy = Xx * Yy;
            ind=IX1+(IY1-1)*mc-1;
            if ((IX1>1)&&(IY1>1)&&(IX1-1<mc-3)&&(IY1-1<nc-3)){
            ...
            * p + 0.4 * O - 0.266666666666667 * a + 6.66666666666667E-02 * d + 0.6 * b -
            ...
            NivGris = XxYy * (CR[0] + Yy * (CR[1] + CR[2] * Yy)
            + XxYy * (CR[5] + CR[6] * Yy + CR[9] * Xx + CR[10] * XxYy)
            + Xx * (CR[8] * Xx + CR[4]));
            NivGris = NivGris + Xx * (CR[3] + Xx * (CR[7] + CR[11] * Xx));
            NivGris = NivGris + Yy * (CR[12] + Yy * (CR[13] + CR[14] * Yy));
            
            *(im1+count)   = NivGris+ CR[15];
            }
            else {
                *(im1+count)   = -(2^16-1);
            }
...
im1= mxGetData(plhs[0]);
    ...
    mexInterpSpline(up,vp,im1,mrows1,ncols1,im0,mrows0,ncols0);
