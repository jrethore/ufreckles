function [Uini3D,M,P]=ExtrudeModel(nmod,Uini,dooperator)
if nargin<3, dooperator=true;end
if nargin<2, Uini=[];end
Uini3D=[];
P=0;
M=0;
check=0;
...
if strcmp(param.basis,'nurbs')
    [Uini3D,M,P]=ExtrudeNURBSModel(nmod,Uini,dooperator);
    return
end
params=param.extrusion_parameters;
method='3d';
% if isfield(param,'extrusion_method')
%     method=param.extrusion_method;
% end

if ~isfield(param0,'calibration_data')
    load(fullfile('TMP',sprintf('%d_mesh_%d',nmod,iscale-1)),'rint','xo','yo','zo','elt','conn','Nnodes','Nelems','Smesh','Vmesh','selected','ng');
    if dooperator
        load(fullfile('TMP',sprintf('%d_phix_%d',nmod,10*(iscale-1))),'phix');
        load(fullfile('TMP',sprintf('%d_phiy_%d',nmod,10*(iscale-1))),'phiy');
        phix=phix(:,(1:prod(Nnodes)));
        phiy=phiy(:,prod(Nnodes)+(1:prod(Nnodes)));
        switch method
            %             case 'average'
            %                 phi0=sparse(size(phix,1),params.nlayers*prod(Nnodes));
            %                 phix=[repmat(phix/params.nlayers,1,params.nlayers),phi0,phi0];
            %                 phiy=[phi0,repmat(phiy/params.nlayers,1,params.nlayers),phi0];
            %                 Nddl_tot=size(phix,2);
            %                 save(fullfile('TMP',sprintf('%d_phix_%d',nmod,10*(iscale-1))),'phix','Nddl_tot','-append');
            ...
            %                 phi1=phix;
            %                 for il=1:(params.nlayers-1)
            %                     phix=blkdiag(phix,phi1);
            %                 end
            %                 phi0=sparse(size(phix,1),params.nlayers*prod(Nnodes));
            %                 phix=[phix,phi0,phi0];
            %
            %                 phi1=phiy;
            %                 for il=1:(params.nlayers-1)
            %                     phiy=blkdiag(phiy,phi1);
            %                 end
            %                 phiy=[phi0,phiy,phi0];
            %
            %
            %                 Xi=repmat(Xi,params.nlayers,1);
            %                 Yi=repmat(Yi,params.nlayers,1);
            %                 if numel(wdetJ)>1
            %                     wdetJ=repmat(diag(wdetJ),params.nlayers,1);
            %                     wdetJ=diag(wdetJ);
            %                 end
            %                 Nddl_tot=size(phix,2);
            ...
            %                 phidfx=phidf(:,(1:prod(Nnodes)));
            %                 phidfy=phidf(:,prod(Nnodes)+(1:prod(Nnodes)));
            %                 phi1=phidfx;
            %                 for il=1:(params.nlayers-1)
            %                     phidfx=blkdiag(phidfx,phi1);
            %                 end
            %                 phi1=phidfy;
            %                 for il=1:(params.nlayers-1)
            %                     phidfy=blkdiag(phidfy,phi1);
            %                 end
            %                 phi0=sparse(size(phix,1),params.nlayers*prod(Nnodes));
            ...
            case '3d'
                ...
                phidfx=phidf(:,(1:prod(Nnodes)));
                phidfy=phidf(:,prod(Nnodes)+(1:prod(Nnodes)));
                phi0=sparse(size(phidfx,1),(params.nlayers-1)*prod(Nnodes));
                %                phidfx=[phidfx,phi0];
                %                phidfy=[phidfy,phi0];
                phidfx=[phidfx,phi0;...
                    phi0,phidfx];
    ...
    if nargout>1
        Mo=diag(sparse(ones(size(phix,2),1)));
        M0=sparse((params.nlayers-2)*prod(Nnodes),(params.nlayers-2)*prod(Nnodes));
        Mx=blkdiag(Mo,blkdiag(M0,Mo));
        My=blkdiag(Mo,blkdiag(M0,Mo));
        %         Mo=diag(sparse(ones(size(phix,2),1)));
        %         M0=sparse((params.nlayers-1)*prod(Nnodes),(params.nlayers-1)*prod(Nnodes));
        %         Mx=blkdiag(Mo,M0);
        %         My=blkdiag(Mo,M0);
    ...
    if numel(params.thickness)==1
        to=0:-params.thickness/(params.nlayers-1):(-params.thickness);
        zo=repmat(to,length(xo),1);
    elseif numel(params.thickness)==length(xo)
        to=0.5:-1/(params.nlayers-1):-0.5;
        zo=params.thickness*to;
    end
    zo=zo(:);
    xo=repmat(xo,params.nlayers,1);
    yo=repmat(yo,params.nlayers,1);
    foundt3=find(elt==3);
    foundq4=find(elt==4);
    conno=conn;
    ...
    %     conn=[conn,repmat(0,size(conn,1),4)];
    %     conn(foundt3,4:6)=conn(foundt3,1:3)+prod(Nnodes);
    %     conn(foundq4,5:8)=conn(foundq4,1:4)+prod(Nnodes);
    elt=2*elt;
    if params.nlayers>2
    ...
    if nargin>1
        Uxini=repmat(Uini((1:size(Uini,1)/2),:),params.nlayers,1);
        Uyini=repmat(Uini(size(Uini,1)/2+(1:size(Uini,1)/2),:),params.nlayers,1);
